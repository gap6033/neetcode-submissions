...
class MedianFinder:
    ...
    def findMedian(self) -> float:
        if (len(self.left_side) + len(self.right_side)) % 2 != 0:
            output = heapq.heappop(self.left_side)
            heapq.heappush(self.left_side, output)
            return -1 * output
        else:
            left_greatest = heapq.heappop(self.left_side)
            right_smallest = heapq.heappop(self.right_side)
            heapq.heappush(self.left_side, left_greatest)
            heapq.heappush(self.right_side, right_smallest)
            return (-1 * left_greatest + right_smallest)/2
